...
chrome.storage.local.get(["content"], function (result) {
    ...
    function s_c(i, n) {
        ...
        let str = '<div class="ta-div"><textarea style=' + textarea_resize + '>' + result.content[i][n] + '</textarea><span class="r-date">' + result.content[i][0] + '</span><a title="' + result.content[i][3] + '" class="round-btn-url"><i></i></a><a title="ダブルクリックで削除" class="round-btn-delete"></a></div>';
        return str;
    }
    try {
